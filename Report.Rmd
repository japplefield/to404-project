...
citi <- merge(citi, weather, by.x = "day", by.y = "Date")
str(citi)
```

Citibike system data was used from [this](https://ride.citibikenyc.com/system-data) data repository. We also used weather data provided from NWS. 
...
tempgenderchart <- ggplot(citi, aes(x = TAVG, fill = gender)) +
    geom_histogram() +
    xlab("Temperature") +
...
```{r}
tempspeedchart <- ggplot(citi, aes(x = TAVG, y = speed)) +
...
```{r}
tempdistchart <- ggplot(citi, aes(x = TAVG, y = distmeters)) +
    geom_point(alpha = 0.2) +
    geom_smooth(method = lm) +
    stat_regline_equation() +
    xlab("Average Temperature") +
...
```{r}
prcpspeedchart <- ggplot(citi, aes(x = PRCP, y = speed)) +
    geom_point(alpha = 0.2) +
    geom_smooth(method = lm) +
    stat_regline_equation() +
    xlab("Precipitation") +
...
```{r}
prcpdistchart <- ggplot(citi, aes(x = PRCP, y = distmeters)) +
    geom_point(alpha = 0.2) +
    geom_smooth(method = lm) +
    stat_regline_equation() +
    xlab("Precipitation") +
...
```{r}
snowgenderchart <- ggplot(citi, aes(x = AWND, fill = gender)) +
    geom_histogram() +
    xlab("Wind") +
    ylab("Number of Trips Taken") +
...
```{r}
tempuserchart <- ggplot(citi, aes(x = AWND, fill = usertype)) +
    geom_histogram() +
    xlab("Wind") +
    ylab("Number of Trips Taken") +
...
```{r}
windspeedchart <- ggplot(citi, aes(x = AWND, y = speed)) +
    geom_point(alpha = 0.2) +
    geom_smooth(method = lm) +
    stat_regline_equation() +
    xlab("Wind") +
...
```{r}
winddistchart <- ggplot(citi, aes(x = AWND, y = distmeters)) +
    geom_point(alpha = 0.2) +
    geom_smooth(method = lm) +
    stat_regline_equation() +
    xlab("Wind") +
...
```{r}
tempagechart <- ggplot(citi, aes(x = TAVG, y = age, color = usertype)) +
    geom_point(alpha = 0.2) +
    geom_smooth(method = lm) +
    stat_regline_equation() +
    xlab("Temperature") +
...
TODO: Make above chart nicer labels

The above histogram depicts the number of rides per day per bike. Most bikes are ridden very infrequently. The number of bikes with more than 2 rides per day is just `r daily_usage %>% filter(num_rides > 2) %>% nrow()`. The number of bikes with more than 3 rides per day is just `r daily_usage %>% filter(num_rides > 3) %>% nrow()`.
...
The above histogram depicts the distribution of average daily rides across all bikes. TODO: Interpret histogram.
...
The above histograms depict the distribution of the number of rides on each bike across each month.
...
Now we broke bike usage out across the entire year to see if some bikes are being ridden more on a year-long basis. The above histogram shows bike usage the number of rides per bikes. TODO: Interpret histogram
...
TODO: Give above map better popups
