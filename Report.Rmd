...
```{r weather-merge}
citi <- inner_join(citi, weather, by = c("day" = "Date"))
str(citi)
```

```{r citi-sample}
citi_sample <- sample_frac(citi, 0.05)
```
...
The above histograms depict the distribution of the number of rides on each bike across each month. It is very intersting to note that in the warmer months between May and November, there are bimodal distributions where a small subset of bikes are being ridden much more frequently than the rest of the bikes. We hypothesize this is due to the effects of higher tourism during those warmer months.
